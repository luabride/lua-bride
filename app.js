...
modal.addEventListener('click',e=>{if(e.target===modal)closeReservation()});
...
document.getElementById('aiForm').addEventListener('submit',e=>{
  ...
  const venue=d.get('venue');
  const style=d.get('style');

  let rec='A LINE';
  let reason='균형 잡힌 실루엣과 자연스러운 체형 보완이 강점입니다.';

  if(venue.includes('스몰웨딩')){
    rec='SIMPLE & MODERN';
    reason='소규모 공간과 가까운 거리에서 보았을 때 자연스럽고 세련된 인상을 주는 가벼운 실루엣과 미니멀한 소재를 추천합니다.';

    if(style.includes('화려')){
      rec='SOFT PRINCESS';
      reason='스몰웨딩 분위기를 해치지 않도록 과한 볼륨은 줄이고, 섬세한 레이스와 은은한 비즈로 로맨틱한 포인트를 살린 드레스를 추천합니다.';
    }
  }else if(venue.includes('웨딩홀웨딩')){
    rec='BALL GOWN';
    reason='웨딩홀 조명과 긴 버진로드, 넓은 무대에서도 존재감이 살아나는 풍성한 볼륨과 반짝임이 있는 드레스를 추천합니다.';

    if(style.includes('심플')){
      rec='A LINE';
      reason='웨딩홀의 규모감은 살리면서도 깔끔하고 정돈된 인상을 주는 실크 A라인 드레스를 추천합니다.';
    }
  }else if(venue.includes('호텔')){
    rec='BALL GOWN';
    reason='호텔의 높은 천장과 고급스러운 조명에 어울리도록 풍성한 실루엣과 섬세한 디테일이 있는 드레스를 추천합니다.';
  }else if(venue.includes('야외')){
    rec='FLOWY A LINE';
    reason='이동이 편하고 자연광과 바람에 아름답게 흐르는 가벼운 소재의 A라인 드레스를 추천합니다.';
  }

  if(style.includes('심플') && !venue.includes('웨딩홀웨딩')){
    rec='SIMPLE & MODERN';
    reason='절제된 선과 깨끗한 소재가 예식 분위기를 세련되게 살립니다.';
  }else if(style.includes('화려') && !venue.includes('스몰웨딩')){
    rec=venue.includes('호텔')||venue.includes('웨딩홀웨딩')?'BALL GOWN':'PRINCESS';
    reason='볼륨감과 디테일이 사진과 공간에서 풍성하게 표현됩니다.';
  }else if(body.includes('상체 슬림') && !venue.includes('스몰웨딩')){
    rec='MERMAID';
    reason='상체와 허리선을 강조해 여성스러운 비율을 만듭니다.';
  }

  const result=document.getElementById('aiResult');
  result.innerHTML=`<b>${rec}</b> 추천<br>${reason}<br><button type="button" class="text-btn" onclick="openReservation()">이 스타일로 피팅 예약하기 →</button>`;
  result.style.display='block';
});
document.getElementById('reservationForm').addEventListener('submit',async e=>{e.preventDefault();const form=e.target,submit=form.querySelector('button[type=submit]'),data=new FormData(form);const reservation={id:makeId(),createdAt:new Date().toISOString(),date:data.get('date'),time:data.get('time'),purpose:data.get('purpose'),name:data.get('name').trim(),phone:data.get('phone').trim(),weddingDate:data.get('weddingDate')||'',memo:data.get('memo')||'',status:'신청'};const result=document.getElementById('reservationResult');submit.disabled=true;submit.textContent='예약 저장 중…';try{if(LuaDataService.mode==='local'){const items=await LuaDataService.list();const duplicate=items.some(r=>r.date===reservation.date&&r.time===reservation.time&&r.status!=='취소');if(duplicate){result.innerHTML='<b>선택한 시간은 이미 예약 신청이 있습니다.</b><br>다른 시간을 선택해 주세요.';result.style.display='block';return}}await LuaDataService.add(reservation);notifyReservation(reservation);result.innerHTML=`<b>예약 신청이 저장되었습니다.</b><br>예약번호: ${reservation.id}<br>${reservation.date} ${reservation.time}<br><small>저장 방식: ${LuaDataService.mode==='firebase'?'온라인 공유 저장':'이 기기 데모 저장'}</small><div class="result-actions"><button type="button" class="secondary small" id="icsBtn">캘린더에 저장</button></div>`;result.style.display='block';document.getElementById('icsBtn').onclick=()=>downloadICS(reservation);form.reset();setMinDates()}catch(err){console.error('Reservation save error:',err);const code=err?.code||'unknown';const message=code==='permission-denied'?'Firestore 보안 규칙에서 고객 예약 등록 권한이 차단되었습니다.':'네트워크 또는 Firebase 설정을 확인해 주세요.';result.innerHTML=`<b>예약 저장에 실패했습니다.</b><br>${message}<br><small>오류 코드: ${code}</small>`;result.style.display='block'}finally{submit.disabled=false;submit.textContent='예약 신청'}});
setMinDates();
